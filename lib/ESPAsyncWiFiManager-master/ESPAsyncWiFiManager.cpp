...
void AsyncWiFiManager::setupConfigPortal() {
  ...
  Serial.println("i am in ESPAsyncWifiNanager.cpp");
...
//start up save config callback
void AsyncWiFiManager::setLogCallback( void (*func)(const char* text) ) {
  _logcallback = func;
}
...
void AsyncWiFiManager::DEBUG_WM(Generic text) {
  if (_debug) {
    Serial.print("*WM: ");
    Serial.println(text);
    String a = "*WM: " + String(text) + "\n";
    if ( _logcallback != NULL) {
      _logcallback(a.c_str());
    }
  }
}
